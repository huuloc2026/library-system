...
export class UserService {
  ...
  async register(body: RegisterNewuserDTO) {
    ...
    const hashedPassword = await hashPasswordHelper(body.password, 8);
    // step 3: send email
    const expiredTime = this.configService.get<number>(
      'MINUTES_TIME_EXPIRES_CODE',
    );
    ...
    const user = await this.databaseService.user.create({
      data: {
        ...
        CodeExpired: dayjs().add(expiredTime, 'minutes').toDate(),
        password: hashedPassword,
      },
    });
    await this.emailsent.sendUserConfirmation(
      user.email,
      user.CodeId,
    );
    return user
